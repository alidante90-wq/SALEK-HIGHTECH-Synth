...
public:
    ...
    void prepare (double sampleRate) { sr = sampleRate > 0.0 ? sampleRate : 44100.0; phase = 0.0; }
    void reset() noexcept { phase = 0.0; lastSH = 0.0f; }
    void setRate (float hz) noexcept { rate = juce::jlimit (0.01f, 40.0f, hz); phaseInc = static_cast<double> (rate) / sr; }
    void setWave (Wave w) noexcept { wave = w; }
    void setAmount (float a) noexcept { amount = juce::jlimit (0.0f, 1.0f, a); }
    float process() noexcept
    {
        float v = 0.0f; const float p = static_cast<float> (phase);
        switch (wave) {
            ...
            case Wave::Random: case Wave::SAndH:
                if (phase < phaseInc) lastSH = juce::Random::getSystemRandom().nextFloat() * 2.0f - 1.0f;
                v = lastSH; break;
        }
        phase += phaseInc; if (phase >= 1.0) phase -= 1.0;
        return v * amount;
    }
private:
    double sr = 44100.0, phase = 0.0, phaseInc = 0.0;
    float rate = 1.0f, amount = 0.0f, lastSH = 0.0f;
    Wave wave = Wave::Sine;
};
