...
public:
    ...
    struct Route { Source source = Source::LFO1; Dest dest = Dest::FilterCutoff; float amount = 0.0f; bool active = false; };
    void clear() noexcept { for (auto& r : routes) r.active = false; }
    int addRoute (Source src, Dest dst, float amount) noexcept {
        for (int i = 0; i < MaxRoutes; ++i)
            if (routes[i].active && routes[i].source == src && routes[i].dest == dst)
            { routes[i].amount = juce::jlimit (-1.0f, 1.0f, amount); return i; }
        for (int i = 0; i < MaxRoutes; ++i)
            if (!routes[i].active) { routes[i] = { src, dst, juce::jlimit (-1.0f, 1.0f, amount), true }; return i; }
        return -1;
    }
    void removeRoute (int slot) noexcept { if (slot >= 0 && slot < MaxRoutes) routes[slot].active = false; }
    void setSourceValue (Source s, float v) noexcept { sourceValues[static_cast<size_t>(s)] = v; }
    float getModulation (Dest d) const noexcept {
        float sum = 0.0f;
        for (const auto& r : routes) if (r.active && r.dest == d) sum += sourceValues[static_cast<size_t>(r.source)] * r.amount;
        return sum;
    }
