...
public:
    ...
    void setCutoff (float hz) noexcept { cutoff = juce::jlimit (20.0f, static_cast<float> (sr * 0.45), hz); update(); }
    void setResonance (float res) noexcept { resonance = juce::jlimit (0.0f, 1.0f, res); update(); }
    void setMode (Mode m) noexcept { mode = m; }
    void setDrive (float d) noexcept { drive = juce::jlimit (0.0f, 1.0f, d); }
    float process (float x) noexcept
    {
        if (drive > 1.0e-4f) { const float g = 1.0f + drive * 3.0f; x = std::tanh (x * g); }
...
private:
    ...
    double sr = 44100.0; float cutoff = 1000.0f, resonance = 0.2f, drive = 0.0f;
    Mode mode = Mode::LowPass; float ic1eq = 0, ic2eq = 0, g = 0, k = 0, a1 = 0, a2 = 0, a3 = 0;
};
